                ...
                <div className='w-[1300px] xl:w-[90%] lg:w-[95%] m-auto flex md:block justify-between mt-20 gap-10 relative'>
                    <div className='w-[270px] md:w-full h-fit md:border border-zinc-800 md:rounded-lg flex-shrink-0 sticky md:relative md:top-0 top-40'>
                        <div className='h-fit rounded-lg p-3 bg-[#171717]'>
                            <h2 className='font-medium text-[#f1f1f1] text-sm leading-tight bg-[#0f0f0f] p-3 rounded-lg'>Compulsory Quarters</h2>
                            <ul className='mt-2 text-[#989797]'>
                                {
                                    ['1', '2', '3'].map((val, index) => {
                                        return (
                                            <Link key={index} href={`/compulsory/${encodeURIComponent(val)}`}><li className={`py-[5px] pl-5 text-sm tracking-widest border-l-[2px] ${params.quarter === val ? 'border-[#01b8b2] text-[#01b8b2]' : 'border-[#0f0f0f]'}`}>Quarter - {val}</li></Link>
                                        )
                                    })
                                }
                            </ul>
                        ...
                        <div className='mt-8 md:p-2'>
                            <h3 className='font-bold text-[#f1f1f1] text-base leading-tight'>Available Tracks</h3>
                            <div className='flex flex-col gap-2 mt-3'>
                                {
                                    tracks.map((val, index) => {
                                        return (
                                            <Link key={index} href={`/tracks/${encodeURIComponent(val.id)}/4`} className='py-2 px-3 leading-none text-sm text-[#f1f1f1] rounded-lg bg-[#171717] transition-all hover:bg-[#01b8b2] hover:text-[#0f0f0f] hover:font-bold'>{val.name}</Link>
                                        )
                                    })
                                }
                            </div>
